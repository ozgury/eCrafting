...
function createUserSession(req, res, user, next) {
  ...
  // Create session
  req.session.user = {username:user.username, fullname: (user.fullname && user.fullname.length) ? user.fullname : user.username, isAdmin:isAdmin, id:user._id, language:user.language, roles:user.roles};
