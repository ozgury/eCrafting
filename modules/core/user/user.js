...
function route(req, res, module, app, next) {
  ...
  // Menu
  res.menu.admin.addMenuItem(req, {name:'Security', path:'security', weight:5, url:'', description:'Users, Roles & Permissions ...', permit:aPerm });
  res.menu.admin.addMenuItem(req, {name:'Users', path:'security/users', weight:10, url:'/user/list', description:'Manage users ...', permit:aPerm });
...
function createUserSession(req, res, user, next) {
  ...
  // Create session
  req.session.user = {username:user.username, email:user.email, fullname: (user.fullname && user.fullname.length) ? user.fullname : user.username, isAdmin:isAdmin, id:user._id, language:user.language, roles:user.roles};
